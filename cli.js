...
const { version } = JSON.parse(fs.readFileSync(join(__dirname, "package.json")));
  ...
  .action(async (query) => {
    ...
    if (query.includes(".")) {
      ...
      const matches = sites.filter(
        (site) =>
          site.primary_domain.includes(query) || site.name.includes(query),
      );
      if (matches.length === 0) {
    ...
    spawn("ssh", [`${environment}@${environment}.ssh.wpengine.net`], {
      stdio: "inherit",
    });
  });
...
wpeCLI
  .command("open <query>")
  .description("Open WPEngine admin panel for environment by name or domain.")
  .action((query) => open(query));
